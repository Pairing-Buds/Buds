...
class ActiveUserTest {
    ...
    @Test
    @DisplayName("활성화 : 비정상 케이스")
    void activate_user_fail(){
        ActiveUserReqDto dto = new ActiveUserReqDto();
        dto.setUserId(10000);

        ApiException apiException = Assertions.assertThrows(ApiException.class, () -> adminService.activeUser(adminId, dto) );
        Assertions.assertEquals(StatusCode.NOT_FOUND, apiException.getCode());
        Assertions.assertEquals(Message.USER_NOT_FOUND, apiException.getMessageEnum());
