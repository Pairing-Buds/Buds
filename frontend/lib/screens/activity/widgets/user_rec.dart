...
class _UserRecState extends State<UserRec> {
                        ...
                        Positioned(
                          ...
                          child: Container(
                            ...
                            child: Column(
                              ...
                              children: _buildTagLines(user.tagTypes, fontSize),
                            ),
                          ),
                        ),
                      ],
  ...
  List<Widget> _buildTagLines(List<String> tags, double fontSize) {
    ...
    // 최대 4개 태그 사용
    final tagList = tags.take(4).toList();

    // 윗줄: 2개, 아랫줄: 1개 (가운데 정렬)
    final firstLine = tagList.take(2).join(', ');
    final secondLine =
        tagList.length > 2 ? tagList.skip(2).take(1).join(', ') : '';
